//#pragma strict
//
////var paused : boolean = false; //the are derailing glitches, etc, from this...
//
//class Muscle
//	{
//	//Methods
...
//			var feign2 : boolean; //necessary afaik
//			var feign3 : boolean;
//			feign3 = feign2;
//			feign2 = feign;
...
//	
//			//The gist of it: if grounded, check .6meters below you, otherwise check 0.05meters down or the distance the player will travel that frame (whichever is farther)
...
//			                     (velocity.y*time < -0.05 ? -velocity.y*time : 0.05),0.6, velocity, grounded); //what's below you?
//			                     
//			                     
//			if(grounded && foot.distance == Mathf.Infinity) Derail(); //if the player stops being grounded, start falling
//			
//			adjMoveDir = Pace(foot.normal,trans.TransformDirection(moveDir)); //Find a suitable direction along a plane
...
//			
//			while(time > 0 && i < length && (i == 0 || n[i-1] != Vector3.zero))  
...
//					var temp2 : Vector3;
//					
//					if     (p[i].y > temp0.y + temp1.y)      temp2 = temp0 + temp1 - p[i];
//					else if(p[i].y < temp0.y - temp1.y)      temp2 = temp0 - temp1 - p[i];
//					else   {                                 temp2 = temp0 - p[i]; temp2.y = 0;}
//					
//					temp2.Normalize();
//					n[i] = temp2;
//					}
//				else n[i] = UltimateIterationArray(position,coll.height/2,coll.radius,velocity.normalized,velocity.magnitude, preVel.magnitude, time,grounded);
//				
...
//						if(Vector3.Dot(velocity,n[i]) < 0)
//							{
//							if(grounded) velocity = Vector3.Project(velocity,Vector3.Cross(n[i],foot.normal).normalized); //project the velocity onto the wall's axis
//							else velocity -=  Vector3.Project(velocity,n[i]); //normally project a velocity along a slope
//							b[i] = true;
//							}
//						else b[i] = false;
...
//								Ground();
//								foot.normal = n[i];
//								var done : boolean = false; //as in "Is it done?"
//								if(i != 0)
//									{
//									for(j = i-1;j > -1;j--) //this last bit was a huge problem, sad part is it works with just the velocity -= part (that was the original, but it felt off/not as fluid)
//										{
//										if(!done && b[j])
//											{
//											Debug.Log("werkz" + Time.time);
//											adjMoveDir = Pace(foot.normal,trans.TransformDirection(moveDir)); //Find a suitable direction along a plane
//                            				if(velocity.sqrMagnitude < baseSpeed*baseSpeed*sprintMult*sprintMult*1.2) Run();
//                            				velocity = Vector3.Project(velocity,Vector3.Cross(n[j],foot.normal).normalized);
//											done = true;
//											}
//										}
//									if(!done)
//										{
//										adjMoveDir = Pace(foot.normal,trans.TransformDirection(moveDir)); 
//										if(velocity.sqrMagnitude < baseSpeed*baseSpeed*sprintMult*sprintMult*1.2) Run();
//										}
//									}
//								else
...
//						if(b[i] && b[j])
//							{
//							if(Neutralize(n[j],n[i],trans.TransformDirection(moveDir)))
//								{
//								velocity.x = 0;
//								velocity.z = 0;
...
//				}
//			
//			if(velocity != Vector3.zero) feign = false;
//			
...
//			if(!feign2 || !feign || !feign3) rigid.MovePosition(position + velocity*time); //position from collisions + the remaining distance*modified velocity
//			else if(foot.normal == Vector3.zero)
//				{
//				position = oldPos;
//				rigid.MovePosition(position + Vector3(0,velocity.y,0)*Time.deltaTime);
//				}
//		} //End FixedUpdate//
//		
//	
//
//	private function IterationArray(pos : Vector3,radius : float, heightHalf : float,dir : Vector3,lowDist : float,highDist : float, vel : Vector3, ground : boolean) : RaycastHit
//		{
//		var hit : RaycastHit;
//		var hit2 : RaycastHit;
//	
...
//			{
//			if(Physics.Raycast(p1,dir,hit,Mathf.Infinity,(1 << 0))
//				&& Physics.Raycast(p1,-hit.normal,hit2,Mathf.Infinity, (1 << 0))
//					&& (hit.distance < radius + 0.05 || hit2.distance < radius + 0.05))//find the normal below us and find how close it is
//						{
...
//						}
//			else if(Physics.SphereCast(p1,radius,dir,hit2,dist,(1 << 0)))
//						{
//						zero = true;
...
//				Ground();
//				if(!zero) place += dir*(hit.distance-0.02/hit.normal.y);
//				else place += dir*(hit.distance-0.02);
//				AssignVel(vel);
//				}
...
//		}
//
//	private function UltimateIterationArray(position : Vector3,heightHalf : float,radius : float,dir : Vector3,speed : float,oSpeed : float, timeNaught : float, ground : boolean) : Vector3
//		{	
//		var hit : RaycastHit;
//		var hit2 : RaycastHit;
//		
//		var p1 : Vector3 = position + (radius-heightHalf)*Vector3.up; //the lower portion //Must be first (info):
//		var p2 : Vector3 = position + (heightHalf-radius)*Vector3.up; //the upper portion //http://forum.unity3d.com/threads/130169-Why-CaspsuleCast-is-buggy
//		
//		var product : Vector3 = dir*oSpeed*timeNaught;
//		
//		var place : Vector3 = position; //the character's position (which is changed in two segments when colliding with walls)
...
//		
//		var zrho : boolean = false;
//		
//		if(Physics.CheckCapsule(p1 + product, p2 + product,radius, (1 << 0))) //if the place the player wants to move to is occupied
//			{
//			if(Physics.Raycast(p1,dir,hit,Mathf.Infinity,(1 << 0)) && Physics.Raycast(p1,-hit.normal,hit2,radius + oSpeed*timeNaught, (1 << 0)) && hit2.distance < 0.022) //check w/ Iteration Array along the floor
//				{
//				hit.distance = hit2.distance - radius;
//				temp = hit.distance/Vector3.Dot(dir,-hit.normal)/speed;
//				
//				if(temp > 0) { temp = 0; reach = timeNaught;}
//				else reach -= temp;
//				
//				if(reach < 0) { reach = 0; place += dir*speed*timeNaught;}
//				else place += dir*temp*speed;
//				}
//			else if(Physics.Raycast(p2,dir,hit,Mathf.Infinity,(1 << 0)) && Physics.Raycast(p2,-hit.normal,hit2,radius + oSpeed*timeNaught, (1 << 0)) && hit2.distance < 0.022) //check w/ Iteration Array along the ceiling
//				{
//				hit.distance = hit2.distance - radius;
//				temp = hit.distance/Vector3.Dot(dir,-hit.normal)/speed;
//				
...
//				{
//				if(hit.normal.y < -0.05) zrho = true;
//				special = true;
//	
...
//			hit.normal = Vector3.zero;
//			}
//		if(!special) place += hit.normal*0.02;
//		//Debug.Log("x: " + place);
//		AssignPos(place);
//		AssignTime(reach);
//		if(zrho && dir.y > 0 && !ground) hit.normal = Vector3(0,-1,0);
//		return hit.normal;
//		}	
//	
//	
...
//		if(Vector3.Dot(temp1,temp2) < 0) return true;
//		
//		if(d1)
//			{
//			temp1 = one;
...
//		
//		temp1 = para - Vector3.Project(para,one);
//		if(Vector3.Dot(temp1,two)  > 0) return true;
//		temp1 -= Vector3.Project(temp1,two);
//		if(Vector3.Dot(temp1,para) < 0) return true;
//		return false;
//		}
//	
//	private function Slide (vel : Vector3,u : Vector3,grav : float, norm : Vector3)
//		{
//		var v : Vector3;
//		v = vel.normalized;
...
//		}
//	
//	private function Pace (normal : Vector3, vect : Vector3)
//		{
...
//	private var feign : boolean; //did the player NOT move last frame?
//	
//	//Floats
//	@System.NonSerialized
...
//	@System.NonSerialized
//	private var baseHeight : float = 4; //the approximate height the player will jump, inaccurate due to progressive gravity
//	@System.NonSerialized
//	private var speed : float = 8; //the real speed of the player
//	@System.NonSerialized
//	private var jumpHeight : float = 4; //the real jump height of the player, high if he got mad hops
//	@System.NonSerialized
...
//	{
//	//Debug.Log(Time.time); //Collisions have only happened in ~.45-.5m tunnels and ~10 cm curbs to date)
//	var pnt : Vector3[];
//	var i : int;
...
//function FixedUpdate ()
//	{
//	muscle.GetMoveDir(Input.GetAxis("Horizontal"),Input.GetAxis("Vertical"));
//	muscle.FixedUpdate();
//	}
//	
//function Update ()
//	{
//	if(Input.GetButtonDown("Jump")) muscle.lastJumpDown = Time.time;
//	}
